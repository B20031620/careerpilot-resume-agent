...
简历工作区是用户进入产品后的第一屏。它应帮助用户快速选择“当前正在处理的简历”，并提供进入核心任务的入口。
...
- 当前简历主卡片。
- 用户上传过的全部简历列表。
- 每份简历的解析状态、创建时间和最近使用时间。
- 上传新简历入口。
- 设为当前简历操作。
- 删除或归档简历操作。
- 从当前简历进入岗位匹配、简历润色、项目包装、模拟面试。
- 当前简历最近生成的分析报告和面试记录。
...
```mermaid
flowchart TD
    A["简历工作区或岗位匹配"] --> B["进入模拟面试"]
    B --> C["默认使用当前简历，也可切换简历和 JD"]
    C --> D["选择面试类型"]
    D --> E["Agent 生成面试计划"]
    E --> F["面试官提问"]
    F --> G["用户回答"]
...
| ---- | ----- | -------- |
| 简历工作区 | 上传/分析 | 上传简历     |
| 简历工作区 | 岗位匹配  | 基于当前简历粘贴 JD |
| 简历工作区 | 模拟面试  | 基于当前简历开始模拟面试 |
| 上传/分析 | 岗位匹配  | 简历解析完成   |
| 岗位匹配 | 简历润色  | 生成优化方案   |
| 岗位匹配 | 模拟面试  | 基于该岗位练习  |
| 简历润色 | 项目包装  | 包装某段项目经历 |
...
```text
请根据 design.md 设计一个中文桌面端 Web App「CareerPilot 职途助手」。这是一个基于 AI Agent 的求职准备产品，包含简历工作区、上传/分析、岗位匹配、简历润色、项目包装、模拟面试、职场沟通和历史报告。请设计成高级、专业、可信赖的 SaaS 产品，不要做营销落地页。第一屏是简历工作区，核心任务是让用户选择当前处理的简历，并允许用户在后续流程中随时切换简历。重点体现页面内容、用户任务流、Agent 执行过程、结构化报告、原文与建议对照、高风险人工确认、多轮模拟面试反馈。所有界面文案必须是简体中文。
```
